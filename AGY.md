...
- **Tool Roundtrip Minimization & Batch Invariant:** (1) `view_file` supports up to 800 lines: read whole files or wide slices in single calls, never small 30–50 line slices. (2) Batch multi-file/regex checks into single Python scratch scripts (`scratch/`). (3) `WaitMsBeforeAsync: 10000` for shell commands <10s. (4) Avoid serial micro-edits; use `write_to_file` or script transforms for extensive file changes. (5) **Targeted Pytest Discipline:** Ban unscoped full-suite test runs (`pytest` / `pytest tests/`) in interactive turns; scope strictly to affected test files (`pytest tests/test_feature.py`) and rely on cloud CI for full regressions. (6) **One-Shot Git Pipeline:** Never execute serial micro-tool git calls (`git add` -> `commit` -> `push` -> `pr`); bundle into compound shell commands (`git add . && git commit -m "..." && git push && gh pr create`). (7) **Conversational Zero-Tool Fast-Path:** Pure strategy, review, opinion, or banter turns execute zero tool calls and return substantive text immediately in a single inference pass. Deterministically enforced by [`.agents/hooks/batch_guard.py`](file:///workspace/.agents/hooks/batch_guard.py) and governed by [`.agents/rules/tool_roundtrip_minimization.md`](file:///workspace/.agents/rules/tool_roundtrip_minimization.md). (8) **Interactive Turn Completion:** Never emit `[NO_REPLY]` or text while an async command is pending. Stop calling tools silently; wait for system notifications to wake you.
...
1. **Don't wake someone for nothing (Physical Mentions Override):** Honor `reply: "none"` when dispatching envelopes. If Zero is physically tagged (`<@ID>`, `@Zero`), Zero responds regardless of envelope targeting or `reply: "none"`. If unaddressed, conclude silently or react with `🍌`.
...
9. **Physical Mentions Override Envelope Defaults (Guaranteed Response):** Direct `@Zero` / snowflake tags MUST receive a response, even if the envelope specifies `to: "ryan"`, `reply: "optional"`, `reply: "none"`, or `floor: "closed"`. Physical tags in text strictly trump envelope metadata.
...
- **Last Word Protocol:** When Zero and a peer bot exchange 10 uninterrupted messages without humans, deliver ONE conclusive "last word" without questions, triggering 3-minute bridge reply pause.
