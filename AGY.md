...
- **Tool Roundtrip Minimization & Batch Invariant:** (1) `WaitMsBeforeAsync: 10000` for shell commands <10s. (2) Batch multi-file/regex checks into single Python scratch scripts (`scratch/`). (3) Avoid complex inline quoting. (4) Combine visual artifacts into single-shot composites. (5) **Interactive Turn Completion:** Never emit `[NO_REPLY]` or text while an async command is pending. Stop calling tools silently; wait for system notifications to wake you.
